...
CP_EXPORT ParserStatus get_status() {
    if (g_parser_status_header == nullptr) {
        return PARSER_STATUS_DONE;
    }
    return static_cast<ParserStatus>(g_parser_status_header->status);
}
...
CP_EXPORT int32_t can_parser_run_worker_segmented(const char* file_path,
    ...
    auto close_segment = [&]() {
        if (seg_handle.addr && seg_hdr) {
            ...
            seg_hdr->status = PARSER_STATUS_DONE;
        }
    ...
    auto open_segment = [&](uint32_t index) -> bool {
        ...
        file_service::init_mmap_header_constract(*seg_hdr, kDataSegmentCapacity, PARSER_STATUS_RUNNING);
        seg_write = 0;
        return true;
    };
    ...
    if (has_index) {
        ...
        auto close_index_segment = [&]() {
            if (idx_handle.addr && ihdr) {
                ...
                ihdr->status = PARSER_STATUS_DONE;
            }
        ...
        auto open_index_segment = [&](uint32_t index) -> bool {
            ...
            ihdr->status = PARSER_STATUS_RUNNING;
        ...
        auto close_channel_index_segment = [&]() {
            if (ch_handle.addr && ch_hdr) {
                ...
                ch_hdr->status = PARSER_STATUS_DONE;
            }
        ...
        auto open_channel_index_segment = [&](uint32_t index) -> bool {
            ...
            ch_hdr->status = PARSER_STATUS_RUNNING;
        ...
        auto close_direction_index_segment = [&]() {
            if (dir_handle.addr && dir_hdr) {
                ...
                dir_hdr->status = PARSER_STATUS_DONE;
            }
        ...
        auto open_direction_index_segment = [&](uint32_t index) -> bool {
            ...
            dir_hdr->status = PARSER_STATUS_RUNNING;
