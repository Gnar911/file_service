...
from file_service.parser.native.can_parser_api import MmapHeaderConstract, ParsedEntry
...
class Record:
    ...
    def __init__(self, record_id: RecordId, base_dir: str | Path, base_name: str):
        ...
        self.__data_handler: CANLogRawDiskFile = CANLogRawDiskFile(
            mmap_dir=str(base_dir),
            mmap_name=token_base_name,
        )
        base_path = Path(base_dir) / token_base_name
        self.__decode_handler: CANLogDecodedDiskFile = CANLogDecodedDiskFile(path=base_path)
    ...
    def get_base_path(self) -> Path:
        return Path(self.__data_handler.mmap_dir) / self.__data_handler.mmap_name
    ...
    def get_total_lines(self) -> int:
        ...
        return int(self.__data_handler.total_lines)

    def get_page_from_row_indices(self, first_line: int, page_size: int) -> List[ParsedEntry]:
        return self.__data_handler.get_page_from_row_indices(first_line, page_size)

    def get_page_from_can_id_row_indices(self, can_id: int, first_line: int, page_size: int) -> List[ParsedEntry]:
        return self.__data_handler.get_page_from_can_id_row_indices(int(can_id), first_line, page_size)

    def get_page_from_can_ids_row_indices(self, can_ids: List[int], first_line: int, page_size: int) -> List[ParsedEntry]:
        return self.__data_handler.get_page_from_can_ids_row_indices([int(cid) for cid in can_ids], first_line, page_size)

    def get_total_count_by_can_ids(self, can_ids: List[int]) -> int:
        return int(self.__data_handler.get_total_count_by_can_ids([int(cid) for cid in can_ids]))

    def get_first_last_timestamp(self) -> Tuple[Optional[float], Optional[float]]:
        return self.__data_handler.get_first_last_timestamp()

    def get_first_last_timestamp_by_can_ids(self, can_ids: List[int]) -> Tuple[Optional[float], Optional[float]]:
        return self.__data_handler.get_first_last_timestamp_by_can_ids([int(cid) for cid in can_ids])

    def get_start_row_by_timestamp(self, timestamp: float) -> int:
        return int(self.__data_handler.get_start_row_by_timestamp(float(timestamp)))

    def get_end_row_by_timestamp(self, timestamp: float) -> int:
        return int(self.__data_handler.get_end_row_by_timestamp(float(timestamp)))

    def get_start_row_by_can_id_timestamp(self, can_id: int, timestamp: float) -> int:
        return int(self.__data_handler.get_start_row_by_can_id_timestamp(int(can_id), float(timestamp)))

    def get_end_row_by_can_id_timestamp(self, can_id: int, timestamp: float) -> int:
        return int(self.__data_handler.get_end_row_by_can_id_timestamp(int(can_id), float(timestamp)))
    ...
    def has_runtime_mmaps(self) -> bool:
        return bool(self.__data_handler.data_segment_paths()) and bool(self.__data_handler.index_segment_paths())

    def get_runtime_mmap_paths(self) -> dict[str, list[Path]]:
        return {
            "data": self.__data_handler.data_segment_paths(),
            "index": self.__data_handler.index_segment_paths(),
            "channel_index": self.__data_handler.channel_index_segment_paths(),
            "direction_index": self.__data_handler.direction_index_segment_paths(),
        }

    def _status_path(self) -> Path:
        data_paths = self.__data_handler.data_segment_paths()
        return data_paths[0] if data_paths else Path(self.__data_handler.data_mmap_path)
    ...
    def get_decode_mmap_paths(self) -> dict[str, list[Path]]:
        ...
        return {
            "signal_dir": self.__decode_handler.decode_signal_dir_segment_paths(),
            "row_index_changed": self.__decode_handler.decode_row_index_changed_segment_paths(),
            "row_index": self.__decode_handler.decode_row_index_segment_paths(),
            "value": self.__decode_handler.decode_value_segment_paths(),
            "rawvalue": self.__decode_handler.decode_rawvalue_segment_paths(),
        }

    def has_decode_mmaps(self) -> bool:
        decode = self.__decode_handler
        seg_methods = (
    ...
    def refresh_runtime(self) -> None:
        if hasattr(self.__data_handler, "refresh_mmap_runtime"):
            self.__data_handler.refresh_mmap_runtime()
        if hasattr(self.__data_handler, "refresh_can_ids_runtime"):
            self.__data_handler.refresh_can_ids_runtime()
        if self.has_decode_mmaps() and hasattr(self.__decode_handler, "refresh_decode_mmap_runtime"):
            self.__decode_handler.refresh_decode_mmap_runtime()

    def save_record(self) -> int:
        data_paths = self.__data_handler.data_segment_paths()
        index_paths = self.__data_handler.index_segment_paths()
        ...
        base_name = self.__data_handler.mmap_name
        ...
        copied = 0
        copied += self._copy_segment_paths(data_paths, self.__data_handler.data_mmap_path, saved_data_path)
        copied += self._copy_segment_paths(index_paths, self.__data_handler.index_mmap_path, saved_index_path)

        channel_paths = self.__data_handler.channel_index_segment_paths()
        direction_paths = self.__data_handler.direction_index_segment_paths()

        if channel_paths:
            copied += self._copy_segment_paths(channel_paths, self.__data_handler.channel_index_mmap_path, saved_channel_index_path)
        if direction_paths:
            copied += self._copy_segment_paths(direction_paths, self.__data_handler.direction_index_mmap_path, saved_direction_index_path)

        if copied > 0:
            self.__data_handler.data_mmap_path = saved_data_path
            self.refresh_runtime()
    ...
    def delete_runtime_mmaps(self) -> int:
        removed = 0
        removed += self._delete_segment_paths(self.__data_handler.data_segment_paths())
        removed += self._delete_segment_paths(self.__data_handler.index_segment_paths())
        removed += self._delete_segment_paths(self.__data_handler.channel_index_segment_paths())
        removed += self._delete_segment_paths(self.__data_handler.direction_index_segment_paths())

        decode_signal_dir_path = self._as_str(getattr(self.__decode_handler, "decode_signal_dir_mmap_path", ""))
        decode_changed_index_path = self._as_str(getattr(self.__decode_handler, "decode_row_index_changed_mmap_path", ""))
        decode_row_index_path = self._as_str(getattr(self.__decode_handler, "decode_row_index_mmap_path", ""))
        decode_value_path = self._as_str(getattr(self.__decode_handler, "decode_value_mmap_path", ""))
        decode_rawvalue_path = self._as_str(getattr(self.__decode_handler, "decode_rawvalue_mmap_path", ""))
    ...
    def get_metadata(self, db_file_path: str | None = None) -> dict[str, Any]:
        ...
        metadata: dict[str, Any] = {
            ...
            "can_ids": [int(cid) for cid in self.__data_handler.get_all_can_ids()],
            "channels": [str(ch) for ch in self.__data_handler.get_all_channels()],
            "time_range": self.__data_handler.get_first_last_timestamp(),
